...
export function hashOf(r) {
  // Only the subtask cell keeps line structure; every other field collapses whitespace exactly as before so hashes stored by earlier syncs stay valid.
  const s = FIELDS.map((f) => (f === "vc"
    ? String(r[f] || "").trim().split(/\r?\n/).map((l) => l.trim().replace(/\s+/g, " ")).join("\n")
    : String(r[f] || "").trim().replace(/\s+/g, " "))).join("␟");
...
export function parseTab(values) {
  ...
  for (let i = hi + 1; i < values.length; i++) {
    ...
    const item = {
      rowNo: i + 1,
      hm: g("hm"), dv: g("dv"), vc: cm.vc !== undefined ? normLines(row[cm.vc]) : "",
...
async function loadTasks(projectId) {
  ...
  // Non-blank subtasks per task, in display order. They all live in the Sheet's single "Việc con" cell, one per line.
  const subsBy = new Map();
  subs
    .filter((s) => (s.text || "").trim())
    .sort((a, b) => (a.position ?? 0) - (b.position ?? 0) || String(a.id).localeCompare(String(b.id)))
    .forEach((s) => { if (!subsBy.has(s.task_id)) subsBy.set(s.task_id, []); subsBy.get(s.task_id).push(s); });
  return { tasks: tasks || [], subsBy };
}
const vcOf = (t, subsBy) => (subsBy.get(t.id) || []).map((s) => s.text.trim().replace(/\s+/g, " ")).join("\n");
const appRow = (t, subsBy) => ({
  hm: t.hm || "", dv: t.dv || "", vc: vcOf(t, subsBy),
  dl: t.dl || "", acc: t.acc || "", st: t.st || ST.N, gc: t.gc || "",
});

// ---------- Sheet -> App, one linked project ----------
async function pullProject(proj, parsed, tab, _unused, stats) {
  const { tasks, subsBy } = await loadTasks(proj.id);
  const writes = [];
  ...
  for (const row of parsed.rows) {
    ...
    if (id) {
      ...
      if (t.sheet_hash && hashOf(appRow(t, subsBy)) !== t.sheet_hash) {
        stats.conflicts.push({ tab, row: row.rowNo, name: row.dv || row.vc });
        continue;
      }
      // Subtasks: line i of the "Việc con" cell <-> i-th subtask. Lines the Sheet dropped are NOT deleted
      // (deleting is an App action); they are written back into the cell so both sides stay identical.
      const lines = row.vc ? row.vc.split("\n") : [];
      const subs = subsBy.get(t.id) || [];
      let nextPos = Math.max(-1, ...subs.map((s) => s.position ?? 0)) + 1;
      for (let i = 0; i < lines.length; i++) {
        if (subs[i]) {
          if (subs[i].text.trim().replace(/\s+/g, " ") !== lines[i]) await db().from("subtasks").update({ text: lines[i] }).eq("id", subs[i].id);
        } else {
          await db().from("subtasks").insert({ task_id: t.id, text: lines[i], st: ST.N, position: nextPos++ });
        }
      }
      const finalVc = [...lines, ...subs.slice(lines.length).map((s) => s.text.trim().replace(/\s+/g, " "))].join("\n");
      if (finalVc !== row.vc && parsed.cm.vc !== undefined) writes.push({ tab, range: `${gs.colLetter(parsed.cm.vc)}${row.rowNo}`, values: [[finalVc]] });
      const { error } = await db().from("tasks").update({
        hm: row.hm, dv: row.dv, dl: row.dl || null, acc: row.acc, st: row.st, gc: row.gc, sheet_hash: hashOf({ ...row, vc: finalVc }),
      }).eq("id", t.id);
  ...
  if (toCreate.length) {
    ...
    created.forEach((t) => {
      ...
      if (item.row.vc) item.row.vc.split("\n").forEach((text, i) => subs.push({ task_id: t.id, text, st: ST.N, position: i }));
    });
    ...
    if (!parsed.idExists && parsed.rows.length) writes.push({ tab, range: `${gs.colLetter(parsed.idCol)}${parsed.headerRowNo}`, values: [["_id"]] });
...
async function pushProjectInner(projectId, { reset = false } = {}) {
    ...
    const { tasks, subsBy } = await loadTasks(projectId);
