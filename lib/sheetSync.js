...
/**
 * Two-way sync of every linked tab, and queue unknown tabs for review.
 * Per linked project: Sheet edits that don't clash are pulled into the App, then the App's state is written to the Sheet.
 * The App is the master: a row changed on both sides ends up with the App's version and is reported.
 */
export async function syncAll() {
  return withLock(async () => {
    ...
    const unknown = tabs.filter((t) => !linkedByTab.has(t.title));
    const values = unknown.length ? await gs.readTabs(unknown.map((t) => t.title)) : {};
    ...
    for (const p of projects || []) {
      if (!tabTitles.has(p.sheet_tab_name)) continue;
      try {
        const r = await pushProjectInner(p.id);
        stats.created += r.created; stats.updated += r.updated; stats.pushed += r.pushed;
        stats.conflicts.push(...r.conflicts); stats.orphans.push(...r.orphans);
      } catch (e) {
        stats.errors.push(`${p.sheet_tab_name}: ${e.message}`);
      }
    }
...
async function pushProjectInner(projectId, { reset = false } = {}) {
  ...
  // Re-read: the pull may have written ids for rows typed in the Sheet.
  let cur = parsed;
  if (!reset) {
    cur = parseTab((await gs.readTabs([tab]))[tab] || []);
    if (!cur) throw new Error(`Tab "${tab}" không đọc được sau khi kéo dữ liệu.`);
  }
  ...
  const lay = parsed2ToList(cur);
  const sameOrder = !reset && cur.idExists && !cur.hasInline && !cur.orphanSubs.length && lay.length === rows.length && rows.every((r, i) => r.id === lay[i].id);
  const cols = Object.entries(cur.cm).map(([f, c]) => [f, c]).sort((a, b) => a[1] - b[1]);
  ...
  await gs.unmergeDataArea(tab, cur.headerRowNo, minC, maxC);

  let written = 0;
  if (sameOrder) {
    // Same rows in the same order: touch only the rows whose content differs (no clearing, so nobody's typing is disturbed).
    const writes = [];
    rows.forEach((r, i) => { if (r.hash !== lay[i].hash) { putRow(writes, lay[i].rowNo, r); written++; } });
    if (writes.length) await gs.writeCells(writes);
  } else {
    // Rows were added / removed / reordered: lay the whole tab out again from the App.
    const start = cur.headerRowNo + 1;
    if (start + rows.length > 1990) throw new Error("Tab quá dài để đồng bộ (tối đa ~1990 dòng).");
    await gs.clearRanges(tab, [...cols.map(([, c]) => c), cur.idCol].map((c) => `${gs.colLetter(c)}${start}:${gs.colLetter(c)}2000`));
    const writes = [];
    if (!cur.idExists) writes.push({ tab, range: `${gs.colLetter(cur.idCol)}${cur.headerRowNo}`, values: [["_id"]] });
    rows.forEach((r, i) => {
      putRow(writes, start + i, r);
      writes.push({ tab, range: `${gs.colLetter(cur.idCol)}${start + i}`, values: [[r.id]] });
    });
    await gs.writeCells(writes);
    written = rows.length;
  }
  if (written) await styleTab(tab, cur);
  await saveMeta(rows);
  return { tab, pushed: written, conflicts: stats.conflicts, created: stats.created, updated: stats.updated, orphans: stats.orphans };
}

/** Readable formatting: dates as dd/mm/yyyy, wrapped text, top-aligned, a hạng mục column wide enough to read. */
async function styleTab(tab, parsed) {
  try {
    const gid = (await gs.listTabs()).find((t) => t.title === tab)?.gid;
    if (gid === undefined) return;
    const c = parsed.cm;
    const defined = (f) => (c[f] !== undefined ? [c[f]] : []);
    await gs.formatTaskColumns(gid, {
      dateCols: defined("dl"),
      textCols: [...defined("hm"), ...defined("dv"), ...defined("vc"), ...defined("gc"), ...defined("acc")],
      otherCols: defined("st"),
    }, parsed.headerRowNo);
    if (c.hm !== undefined) await gs.ensureMinWidth(gid, tab, c.hm, 200);
    if (c.dl !== undefined) await gs.ensureMinWidth(gid, tab, c.dl, 105);
  } catch {
    // cosmetic only — never fail a sync over formatting
  }
}

const parsed2ToList = (p) => p.groups.flatMap((g) => [{ id: g.t.id, hash: hashT(g.t), rowNo: g.t.rowNo }, ...g.subs.map((s) => ({ id: s.id, hash: hashS(s), rowNo: s.rowNo }))]);
