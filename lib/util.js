import { ST, STO, R } from "./constants";
...
// User role only sees tasks in projects they chủ trì; admin/super see everything.
export function scopedTasks(D, me) {
  if (me.role !== R.U) return D.tasks;
  const own = new Set(D.projects.filter((p) => p.owner === me.id).map((p) => p.id));
  return D.tasks.filter((t) => own.has(t.pid));
}
...
export function rowSt(t) {
  if (done(t)) return { bg: "bg-slate-100/70", bd: "border-l-4 border-l-slate-300", fade: true };
  const u = urg(t);
  if (u === "over") return { bg: "bg-rose-100/70", bd: "border-l-4 border-l-rose-600", fade: false };
  if (u === "today") return { bg: "bg-orange-100/60", bd: "border-l-4 border-l-orange-500", fade: false };
  if (u === "soon") return { bg: "bg-amber-100/50", bd: "border-l-4 border-l-amber-400", fade: false };
  return { bg: "bg-white", bd: "border-l-4 border-l-transparent", fade: false };
}
