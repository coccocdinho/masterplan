...
export default function SubRow({ t, s, onUpdSub, onDelSub }) {
  ...
  return (
    <tr className="border-b border-slate-100 last:border-0 align-top bg-slate-50/40">
      ...
      <td className="px-2 py-1"><AutoTextarea value={s.acc||""} onChange={e=>onUpdSub(t.id,s.id,{acc:e.target.value})} className={`w-full rounded border border-transparent bg-transparent px-1 py-0.5 hover:border-slate-200 focus:border-indigo-400 focus:bg-white focus:outline-none ${sdn?"text-slate-400":""}`}/></td>
